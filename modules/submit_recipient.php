...
try {
    ...
    $checkStmt = $conn->prepare("SELECT id FROM email_recipients WHERE email = ?");
    ...
    $stmt = $conn->prepare("INSERT INTO email_recipients (nama, email, status, created_at, updated_at) VALUES (?, ?, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
